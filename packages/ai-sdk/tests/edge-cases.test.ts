...
import { memoryToolInputSchema } from "../src/schemas/memory-tool-schema";
import { buildMemoryToolDefinition } from "../src/tools/build-memory-tool-definition";
import { runStructuredMemoryTool } from "../src/tools/run-structured-memory-tool";
...
test("buildPrepareCallMemoryText assembles context correctly", async () => {
	...
	const text = await buildPrepareCallMemoryText({
		...
		retrievalPlan: { allowedScopes: ["workspace"], readUserMemory: false, readArchivalMemory: true, includeRecall: false, precedence: ["workspace"] },
	});
