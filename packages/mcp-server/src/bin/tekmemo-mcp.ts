...
async function main(): Promise<void> {
	...
	try {
		runtime = await createTekMemoMcpRuntimeFromConfig({
			...
			cloud: {
				...
				workspaceId: args.workspaceId as string | undefined,
				projectId: args.projectId as string | undefined,
				timeoutMs: numberArg(args.cloudTimeoutMs, undefined),
			},
		});
	} catch (error) {
		console.error(
	...
	const readOnly =
		args.readOnly ?? process.env.TEKMEMO_MCP_READ_ONLY === "true";
	const server = createTekMemoMcpProtocolServer({
		runtime,
		readOnly,
		requestTimeoutMs: numberArg(args.requestTimeoutMs, 30_000),
		maxInputBytes: numberArg(args.maxInputBytes, 256_000),
		maxOutputBytes: numberArg(args.maxOutputBytes, 512_000),
	});
