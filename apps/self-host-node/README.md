...
It runs the shared Hono app from `@tekbreed/tekmemo-server` and uses:
...
pnpm --filter @tekbreed/tekmemo-self-host-node migrate
pnpm --filter @tekbreed/tekmemo-self-host-node dev
```

For Railway/Fly-style hosts, deploy the Dockerfile once and run two processes from the same image:

```bash
pnpm --filter @tekbreed/tekmemo-self-host-node server
pnpm --filter @tekbreed/tekmemo-self-host-node worker
```
